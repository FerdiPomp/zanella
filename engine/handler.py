...
def _emit_qr_append(event_queue, shared_qr_state, node_id, qr, timestamp):
    shared_qr_state.update(qr, timestamp)
    _queue_event(event_queue, node_id, "QR_APPEND", timestamp, qr=qr)


def _emit_qr_removed(event_queue, shared_qr_state, node_id, qr, timestamp):
        ...
        if state == CONFIG.NO_QR:
            if qr is not None:
                ...
                if pending_since is None:
                    ...
                    state = CONFIG.QR
                    pending_since = None
                    _emit_qr_append(event_queue, shared_qr_state, node_id, qr, now)
            else:
                pending_since = None
                last_qr = None
